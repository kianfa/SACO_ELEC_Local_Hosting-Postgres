          ...
          <CardContent className="space-y-3">
            ...
            {variants.length ? variants.map((variant, index) => (
              <div key={variant.id ?? index} className="grid gap-2 rounded-xl border p-3 md:grid-cols-[1fr_1fr_110px_90px_auto]">
                ...
                <Field label="قیمت" error={state.fieldErrors?.[`variants.${index}.price`]}><Input value={variant.price} onChange={(event) => updateVariant(index, "price", Number(event.target.value.replace(/,/g, "")) || 0)} inputMode="numeric" className="rounded-xl" /></Field>
