...
function mapBanner(row: RawBanner): SiteBanner {
  return {
    ...
    description: row.description ?? null,
    imageUrl: normalizePublicMediaUrl(row.image_url),
    imageAltText: row.image_alt_text?.trim() || row.title || "بنر الکتروساکو",
...
export async function deleteBanner(id: string): Promise<void> {
  ...
  if (imageUrl) {
    ...
    if ((shared.count ?? 0) === 0) {
      const { deleteMediaByPublicUrl } = await import("@/lib/storage/media-storage")
      await deleteMediaByPublicUrl(imageUrl)
    }
  }
...
export async function uploadSiteMedia(file: File, folder: string, fileName: string): Promise<string> {
  const { uploadMedia } = await import("@/lib/storage/media-storage")
