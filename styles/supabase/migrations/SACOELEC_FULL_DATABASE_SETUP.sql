...
-- Standalone PostgreSQL setup for ElectroSaco.
...
create table if not exists public.product_documents (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint product_documents_storage_path_check check (btrim(storage_path) <> '')
);
...
create index if not exists product_documents_product_id_sort_idx on public.product_documents(product_id, sort_order, created_at);
...
begin
  for item in
    select * from (values
      ...
      ('product_documents', 'update_product_documents_updated_at'),
...
create policy "public read active product documents" on public.product_documents for select to public
using (exists (select 1 from public.products p where p.id = product_documents.product_id and p.is_active = true));
...
begin
  foreach table_name in array array[
    'brands','categories','products','product_variants','product_images','product_specs','product_documents','inventory',
...
grant select on table public.brands, public.categories, public.products, public.product_variants,
  public.product_images, public.product_specs, public.product_documents, public.inventory, public.homepage_sections,
...
revoke insert, update, delete on table public.brands, public.categories, public.products,
  public.product_variants, public.product_images, public.product_specs, public.product_documents, public.inventory,
...
-- Allow authenticated Supabase admin sessions to manage document rows; RLS still
-- restricts writes to public.is_admin(). Plain PostgreSQL app-server access is
-- covered by the sacouser policy above.
grant insert, update, delete on table public.product_documents to authenticated;

-- Optional Supabase Storage setup for product PDF documents. Current app storage
-- uses local/MinIO media drivers, but this bucket is ready for Supabase Storage.
